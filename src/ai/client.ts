...
function getFallbackCandidates(current: ProviderType, config: HysaConfig): FallbackCandidate[] {
    ...
    async sendMessage(messages: Message[], systemPrompt: string, signal?: AbortSignal): Promise<AIResponse> {
      ...
      const tryProvider = async (provider: ProviderType, model: string, timeoutMs: number, attemptLabel: string): Promise<AIResponse | null> => {
        if (shouldSkipProvider(provider, model)) {
          ...
          if (debug) console.log(`  [debug] ${skippedReasons[skippedReasons.length - 1]}`);
          return null;
        }
      ...
      if (debug && lastError) {
        ...
        if (skippedReasons.length > 0) {
          ...
          for (const sr of skippedReasons) console.log(sr);
        }
      }

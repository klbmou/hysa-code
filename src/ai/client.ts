...
export function categorizeError(msg: string): ErrorCategory {
  ...
  if (lower.includes('econnrefused') || lower.includes('econnreset') || lower.includes('fetch failed') || lower.includes('network') || lower.includes('enotfound') || lower.includes('econnaborted') || lower.includes('connection')) return 'network';
...
function categorizeProviderError(provider: ProviderType, err: unknown): ErrorCategory {
  if (provider === 'ninerouter') {
    return ninerouterProbeStatusToErrorCategory(classifyNinerouterFailure(err));
  }
  return categorizeError((err as Error)?.message || String(err));
}

function logNinerouterFailure(reqId: number, model: string, err: unknown, cat: ErrorCategory, debug: boolean): void {
  ...
  if (debug) console.log(`[req:${reqId}] 9Router model failed: ${model} - ${cat}`);
}
...
function friendlyRateLimit(provider: ProviderType): string {
...
function friendlyError(msg: string, provider: ProviderType): string {
  ...
  if (cat === 'rate_limit') return friendlyRateLimit(provider);

  if (cat === 'quota') {
    if (provider === 'openrouter') return `${label} requires credits or a paid plan for this model. Try a free model like qwen/qwen3-coder:free.`;
  ...
  if (cat === 'network') {
    ...
    if (provider === 'openrouter') return `Cannot reach ${label}. Check your internet connection or OpenRouter may be down.`;
    return `Cannot reach ${label}. Check your internet connection.`;
  }

  if (cat === 'invalid_key') {
    if (provider === 'opencode_zen') return `${label} API key is invalid or missing. Get a key from https://opencode.ai/zen`;
  ...
  if (cat === 'model_unavailable') {
    if (provider === 'gemini') return `${label} free tier is temporarily overloaded (503). Provider-side issue, not your context. Trying another provider...`;
    if (provider === 'openrouter') return `${label} model is temporarily unavailable. Trying another model or provider...`;
...
export function getFallbackCandidates(current: ProviderType, config: HysaConfig): FallbackCandidate[] {
  ...
  if (tier === 'free_api') {
    ...
    for (const model of ninerouterModels) add('ninerouter', model);
  ...
  if (tier === 'local_free') {
    ...
    if (config.apiKeys.gemini) add('gemini');
    if (config.apiKeys.deepseek) add('deepseek');
    if (config.apiKeys.opencode_zen) add('opencode_zen');
    if (config.apiKeys.groq) add('groq');
  }

  // C. Premium fallback
  if (tier === 'premium_api') {
    for (const p of PREMIUM_API_PROVIDERS) {
      if (p !== current && config.apiKeys[p as keyof typeof config.apiKeys]) add(p);
    }
  ...
  if (tier === 'experimental_free') {
    if (config.allowExperimentalProviders) {
      ...
      if (current !== 'pollinations') add('pollinations');
      if (current !== 'llm7') add('llm7');
      if (current !== 'puter') add('puter');
    }
...
function extractDebugInfo(err: unknown, provider: ProviderType, model: string, config: HysaConfig, elapsed?: string, timeoutMs?: number, attempt?: number): string {
  const lines: string[] = [];
  const cat = categorizeError((err as Error)?.message || '');
  ...
  else if (e.response?.status) lines.push(`  HTTP Status: ${e.response.status}`);
  if (e.message) {
...
function tryCreateClient(provider: ProviderType, model: string, config: HysaConfig): AIClient | null {
  try {
...
function createFallbackClient(primary: ProviderType, config: HysaConfig): AIClient {
    ...
    async sendMessageStream(messages: Message[], systemPrompt: string, onEvent: (event: StreamEvent) => void, signal?: AbortSignal): Promise<AIResponse> {
      ...
      if (providerModelHasActiveCredentials(primary, primaryModel, config) && !shouldSkipProvider(primary, primaryModel)) {
        ...
        if (client?.sendMessageStream) {
          ...
          if (config.debug) console.log(`[req:${streamReqId}] Streaming ${provLabel} / ${primaryModel}...`);
  ...
  async function sendMessageFallback(messages: Message[], systemPrompt: string, signal?: AbortSignal): Promise<AIResponse> {
          ...
          try {
            ...
            if (!result.message?.trim() && (!result.toolCalls || result.toolCalls.length === 0)) {
              ...
              if (isExperimental && retries === 0) {
                ...
                if (debug) console.log(`[req:${reqId}] ${provLabel} empty response, retrying once`);
                clearTimeout(timer);
            ...
            if (cat === 'timeout') {
              ...
              if (provider === 'openrouter' && attemptLabel === 'Primary') {
                ...
                if (debug) console.log(`[req:${reqId}] OpenRouter global 429 detected — will skip remaining OR models`);
              }
      ...
      if (debug) console.log(`[req:${reqId}] Trying primary: ${primary} / ${primaryModel} (timeout: ${primaryTimeout / 1000}s)`);
      ...
      if (primary === 'openrouter') {
        ...
        for (const altModel of orderedModels) {
          if (openRouterGlobal429) {
            if (debug) console.log(`[req:${reqId}] OpenRouter global 429 — skipping model fallback ${altModel}`);
            break;
          }
          if (triedModels.has(altModel)) continue;
          triedModels.add(altModel);
          ...
          if (shouldSkipProvider(primary, altModel)) {
            if (debug) console.log(`[req:${reqId}] Skipping unhealthy model: ${altModel}`);
            continue;
          }

          if (debug) console.log(`[req:${reqId}] Model fallback to ${altModel}`);
      ...
      for (const group of groups) {
        ...
        if (allSkipped) {
          ...
          if (debug) console.log(`[req:${reqId}] ${info}`);
        ...
        for (const model of group.models) {
          if (shouldSkipProvider(group.provider, model)) {
            if (debug) console.log(`[req:${reqId}]  skipping model: ${model} (unhealthy)`);
            continue;
          }
          if (Date.now() - startTime >= MAX_TOTAL_TIME_MS) break;

          const fbTimeout = getProviderTimeout(group.provider, false);
          if (debug) console.log(`[req:${reqId}]  trying: ${group.provider}/${model} (timeout: ${fbTimeout / 1000}s)`);

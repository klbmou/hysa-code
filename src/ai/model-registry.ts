...
import {
  getProviderPreferenceForTask,
  providerModelHasActiveCredentials,
...
const TASK_PRIORITY_ORDER: Record<string, string[]> = {
...
export function providerIsAvailable(provider: string, config: HysaConfig): boolean {
...
export function getCandidatesForTask(
...
): { provider: string; model: string; label: string; priority: string }[] {
  const candidates: { provider: string; model: string; label: string; priority: string }[] = [];
  ...
  for (const pri of priorityOrder) {
    for (const mc of registry) {
      if (mc.priority !== pri) continue;

      const isConfiguredModel = mc.provider === config.currentProvider && mc.model === config.currentModel;

      if (normalizedTask === 'image_vision') {
        if (!isConfiguredModel && !mc.capabilities.includes('image_vision')) continue;
      } else if (normalizedTask !== 'unknown') {
        if (!isConfiguredModel && !mc.capabilities.includes(normalizedTask)) continue;
      } else {
        if (!isConfiguredModel && !mc.capabilities.includes('unknown')) continue;
      }
      ...
      if (!providerIsAvailable(mc.provider, config)) continue;
      if (!providerModelHasActiveCredentials(mc.provider, mc.model, config)) continue;
      if (mc.provider !== 'ninerouter' && healthChecker.isProviderOnCooldown?.(mc.provider)) continue;
      if (healthChecker.isOnCooldown(mc.provider, mc.model)) continue;
      if (healthChecker.isUnhealthy(mc.provider, mc.model)) continue;
  ...
  return candidates.sort((a, b) => {
    ...
    if (a.provider === config.currentProvider && b.provider === config.currentProvider) {
      if (a.model === config.currentModel && b.model !== config.currentModel) return -1;
      if (a.model !== config.currentModel && b.model === config.currentModel) return 1;
    }
...
export function getSkippedProviderReasons(config: HysaConfig): { provider: string; reason: string }[] {
  const reasons: { provider: string; reason: string }[] = [];
  const checks: { provider: string; check: () => boolean; reason: string }[] = [
...
function normalizeTaskForRegistry(taskKind: TaskKind): TaskKind {
  switch (taskKind) {
    case 'debugging':
    case 'code_review':
      return 'code_edit';
...
function getRuntimeRegistry(config: HysaConfig, runtimeModels?: RuntimeProviderModels): ModelCapability[] {
  ...
  for (const model of ninerouterModels) {
    if (!model || registry.some(m => m.provider === 'ninerouter' && m.model === model)) continue;
    registry.push({
      provider: 'ninerouter',
      model,
      label: `9Router / ${model}`,
  ...
  for (const model of ninerouterVisionModels) {
    ...
    if (existing) {
      if (!existing.capabilities.includes('image_vision')) existing.capabilities.push('image_vision');
      continue;
    }
    registry.push({
      provider: 'ninerouter',
...
function looksVisionModel(model: string): boolean {
  return /(gemini|gpt-4o|vision|vl|image|multimodal)/i.test(model);
}

function ollamaPriority(model: string): ModelCapability['priority'] {
  const lower = model.toLowerCase();
  if (/(0\.5b|1\.5b|3b|mini|tiny|flash|fast)/.test(lower)) return 'fast';
  if (/(coder|code|deepseek|qwen|starcoder|codestral)/.test(lower)) return 'balanced';
  return 'fast';
}

function providerIndex(providerOrder: ProviderType[], provider: string): number {
  const index = providerOrder.indexOf(provider as ProviderType);
  return index >= 0 ? index : providerOrder.length + 1;
}

function getConfiguredNinerouterOrder(config: HysaConfig, runtimeModels?: RuntimeProviderModels): string[] {
...
function modelIndex(ordered: string[], model: string): number {
  const index = ordered.indexOf(model);
  return index >= 0 ? index : ordered.length + 1;
}

function priorityIndex(priorityOrder: string[], priority: string): number {

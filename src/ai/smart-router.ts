...
import {
  ...
  ninerouterProbeStatusToErrorCategory,
} from './ninerouter.js';
...
export function createSmartRouter(config: HysaConfig, _signal?: AbortSignal): AIClient {
  const debug = !!config.debug;
  const isWeb = !!process.env.HYSA_WEB_MODE || false;

  function getMaxAttempts(): number {
    if (isWeb) return getEnvInt('HYSA_WEB_MODEL_MAX_ATTEMPTS', 6);
    return getEnvInt('HYSA_MODEL_MAX_ATTEMPTS', 6);
  }

  const router: AIClient = {
    async sendMessage(messages: Message[], systemPrompt: string, signal?: AbortSignal): Promise<AIResponse> {
      ...
      const providerStats = new Map<string, ProviderAttemptStats>();
      ...
      for (let i = 0; i < attempts.length; i++) {
        ...
        if (c.provider !== 'ninerouter' && isProviderOnCooldown(c.provider)) {
          if (!skippedProviders.has(c.provider)) {
            ...
            if (debug) console.log(`${LOG}[req:${reqId}] Skipping ${c.provider}: provider cooldown ${remaining}s`);
          }
          continue;
        }
        ...
        if (c.provider === 'ninerouter') {
          ...
          if (ninerouterModelFailures > 0) addFallbackEvent(c.provider, c.model, `Trying next 9Router model: ${c.model}`);
          if (debug) console.log(`${LOG}[req:${reqId}] ${msg}`);
        }
        ...
        try {
          ...
          if (c.provider === 'ninerouter' && ninerouterModelFailures > 0) {
            ...
            if (debug) console.log(`${LOG}[req:${reqId}] selected 9Router fallback model: ${c.model}`);
          }
    ...
    async sendMessageStream(messages: Message[], systemPrompt: string, onEvent: (event: StreamEvent) => void, signal?: AbortSignal): Promise<AIResponse> {
      ...
      const client = createSingleClient(
        first.provider as ProviderType,
        first.model,
        config.apiKeys,
        config.ollamaBaseUrl,
        config.localOpenAiBaseUrl,
...
Previous partial response: ${partialTokens.slice(0, 500)}`;
          ...
          const sameModelClient = createSingleClient(
            first.provider as ProviderType,
            first.model,
            config.apiKeys,
            config.ollamaBaseUrl,
          ...
          try {
            ...
            const nsMsg = (nsErr as Error).message || '';
...
interface ProviderAttemptStats {
  attempts: number;
...
export function applyTaskBasedRouting(
  candidates: RouterCandidate[],
  taskKind: TaskKind,
  userText: string,
  config: HysaConfig,
  _runtimeModels?: RuntimeProviderModels,
): RouterCandidate[] {
  const isArabic = userText.length > 0 && isArabicText(userText);
  const codeProjectTasks: TaskKind[] = ['code_edit', 'code_review', 'debugging', 'project_scan', 'coding_qa', 'long_reasoning'];
  ...
  const isGeminiCandidate = (c: RouterCandidate) => c.provider === 'ninerouter' && /gemini/i.test(c.model);

  function healthyGemini(cands: RouterCandidate[]): RouterCandidate[] {
...
export function buildAttemptPlan(candidates: RouterCandidate[], taskKind: TaskKind, maxAttempts: number): RouterCandidate[] {
  const groups = new Map<string, RouterCandidate[]>();
  for (const candidate of candidates) {
    const group = groups.get(candidate.provider);
    if (group) group.push(candidate);
    else groups.set(candidate.provider, [candidate]);
  }
  ...
  for (const [provider, group] of groups) {
    ...
    for (const candidate of group.slice(0, limit)) {
      if (provider !== 'ninerouter') {
        if (nonNinerouterAttempts >= nonNinerouterLimit) continue;
...
function getProviderAttemptLimit(provider: string, taskKind: TaskKind): number {
  if (provider === 'ninerouter') return getEnvInt('HYSA_9ROUTER_MAX_MODEL_ATTEMPTS', 8);
  if (taskKind === 'simple_chat') return provider === 'ollama' ? 2 : 2;
  if (provider === 'openai_router' && (taskKind === 'code_edit' || taskKind === 'debugging' || taskKind === 'code_review')) return 4;
  if (provider === 'openrouter') return 3;
  return 2;
}

function countProviderTargets(candidates: RouterCandidate[]): Map<string, number> {
...
function recordProviderFailure(
  statsByProvider: Map<string, ProviderAttemptStats>,
  provider: string,
  category: ErrorCategory,
  plannedProviderAttempts: number,
): string | null {
...
function categorizeProviderError(provider: string, err: unknown): ErrorCategory {
  if (provider === 'ninerouter') {
...
function logNinerouterFailure(reqId: number, model: string, err: unknown, category: ErrorCategory, debug: boolean): void {
  const details = extractNinerouterErrorDetails(err);
  const fields = [
    `status=${details.httpStatus ?? 'unknown'}`,
    `error.type=${details.errorType ?? 'unknown'}`,
    `error.message=${truncate(details.errorMessage ?? (err as Error)?.message ?? String(err), 300)}`,
    'provider=ninerouter',
    `model=${model}`,
  ...
  if (debug) console.log(`${LOG}[req:${reqId}] 9Router model failed: ${model} - ${category}`);
}
...
function categorizeError(msg: string): ErrorCategory {
  ...
  if (lower.includes('401') || lower.includes('unauthorized') || lower.includes('auth') || lower.includes('invalid key') || lower.includes('403')) return 'invalid_key';
  if (isRateLimitError(msg)) return 'rate_limit';
  if (lower.includes('quota') || lower.includes('402') || lower.includes('payment') || lower.includes('billing')) return 'quota';
  if (isTimeoutError(msg)) return 'timeout';
  if (lower.includes('econnrefused') || lower.includes('econnreset') || lower.includes('fetch failed') || lower.includes('network') || lower.includes('enotfound') || lower.includes('connection')) return 'network';
  if (lower.includes('404') || lower.includes('model not found') || lower.includes('does not support') || lower.includes('not support') || lower.includes('unavailable') || lower.includes('empty') || lower.includes('503')) return 'model_unavailable';
  return 'unknown';
}

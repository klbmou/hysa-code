...
function sanitize(text: string, maxLen = MAX_SUMMARY_LENGTH): string {
  let cleaned = text.trim();
  if (containsSecret(cleaned)) return '[REDACTED]';
  if (cleaned.length > maxLen) cleaned = cleaned.slice(0, maxLen) + '...';
  return cleaned;
}

function makeTitle(kind: string, ...parts: string[]): string {
  const label = `${kind}: ${parts.filter(Boolean).join(' — ')}`;
  return label.slice(0, MAX_TITLE_LENGTH);
}

function makeTags(...extra: string[]): string[] {
  return ['persistent-memory', ...extra.filter(Boolean)];
}

// ── Importance scoring helpers ──

function scoreImportance(kind: 'lesson' | 'decision', source: MemorySource, text: string): number {
  let base = kind === 'decision' ? 70 : 60;
  // Boost for manual entries
  if (source === 'manual') base += 10;
  if (source === 'user') base += 5;
  // Longer, more detailed descriptions are more important
  if (text.length > 100) base += 5;
  if (text.length > 300) base += 5;
  return Math.min(base, 100);
}

function scoreConfidence(kind: 'lesson' | 'decision', source: MemorySource): number {
  switch (source) {
    case 'user': return 85;
...
export async function writeMemory(
  ...
  const existing = similar.nodes.find(
    n => n.kind === kind && (n.label === safeTitle || n.summary === safeSummary),
  );
  if (existing) {
...
export async function writeAutoFixMemory(
  fixResult: {
    fixed: boolean;
    errorType: string;
    filesTouched: string[];
...
export function classifyMemoryText(text: string): { kind: 'lesson' | 'decision' | null; content: string } {
...
export async function writeMemoryFromText(text: string): Promise<{ kind: string; title: string } | null> {
...
export function containsMemoryTrigger(text: string): boolean {

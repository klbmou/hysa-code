...
const AUTO_FIX_TASK_KINDS = new Set<TaskKind>([
  'code_edit', 'coding_qa', 'debugging', 'project_scan',
  'code_review', 'skill_task',
]);

export function isAutoFixTask(taskKind: TaskKind): boolean {
  return AUTO_FIX_TASK_KINDS.has(taskKind);
}

export function classifyError(result: string, toolType: string): ErrorDetails {
...
export function extractFileInfo(text: string): { filePath?: string; line?: number; column?: number } {
  let m: RegExpExecArray | null;

  m = TS_ERROR_RE2.exec(text);
  if (m) return { filePath: m[1], line: parseInt(m[2], 10), column: parseInt(m[3], 10) || undefined };

  m = TS_ERROR_RE.exec(text);
  if (m) return { filePath: m[1], line: parseInt(m[2], 10), column: m[3] ? parseInt(m[3], 10) : undefined };

  m = ERROR_AT_RE.exec(text);
  if (m) return { line: parseInt(m[1], 10), column: parseInt(m[2], 10) };

  m = FILE_IN_ERROR_RE.exec(text);
  if (m) return { filePath: m[1] };
...
export function isFixableError(details: ErrorDetails): boolean {
  if (!details.filePath) return false;
  if (details.category === 'command_timeout') return false;
  if (details.category === 'permission_error') return false;
  return true;
}
...
function computeErrorHash(result: string): string {
  const lines = result.split('\n').filter(l => /error|Error|TS\d+|fail|Fail|✗|✘/.test(l));
  return lines.slice(0, 3).join('|').replace(/\s+/g, ' ').slice(0, 200);
}

export function shouldAutoFix(result: string, toolType: string, taskKind: TaskKind): boolean {
  if (!isAutoFixTask(taskKind)) return false;
  if (toolType !== 'execute_command') return false;
...
export async function attemptAutoFix(
  ...
  workingDir: string,
  userMessage: string,
  state: { attempts: number; lastErrorHash: string },
  runCommand: (cmd: string) => { stdout: string; stderr: string },
  debug?: boolean,
): Promise<{
  fixed: boolean;
  newResult?: string;
  ...
  try {
    const aiResult = await client.sendMessage(
...
function isLikelyProtected(filePath: string): boolean {
  const base = filePath.split(/[\/\\]/).pop() || '';
  if (/^\.env($|\.)/i.test(base)) return true;
  if (/\b(secret|key|token)\b/i.test(base)) return true;
  if (/node_modules[/\\]/i.test(filePath)) return true;
  if (/[/\\](dist|build|out)[/\\]/i.test(filePath)) return true;
  return false;
}

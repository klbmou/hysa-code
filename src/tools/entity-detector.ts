...
function isCommonProgrammingConcept(name: string): boolean {
...
export function looksLikeHandle(s: string): boolean {
  ...
  if (SHORT_ENTITY_WORDS.has(trimmed.toLowerCase())) return false;
  if (isCommonProgrammingConcept(trimmed)) return false;

  if (trimmed.length > 20) return false;

  if (/^[a-zA-Z0-9_.-]+$/.test(trimmed) && /[a-zA-Z]/.test(trimmed)) return true;

  if (/^[A-Z][a-z]+[A-Z]/.test(trimmed)) return true;

  if (/[A-Z]/.test(trimmed) && /[a-z]/.test(trimmed) && /^[a-zA-Z0-9]+$/.test(trimmed)) return true;
...
function looksLikeName(s: string): boolean {
  ...
  if (/^[^a-zA-Z0-9\u0600-\u06FF\u4E00-\u9FFF\u0400-\u04FF]+$/.test(trimmed)) return false;

  const lower = trimmed.toLowerCase();

  // Common entity words
  if (SHORT_ENTITY_WORDS.has(lower)) return false;

  // Greetings
  if (/^(hi|hello|hey|salam|مرحبا|thanks|thank you|ok|okay|goodbye|bye)$/i.test(trimmed)) return false;
  ...
  if (!trimmed.includes(' ') && trimmed.length >= 3) {
    if (/^[a-z][a-z0-9_.-]+$/.test(trimmed) && /[a-z]{3}/.test(trimmed)) return true;
    if (/^[a-zA-Z]+$/.test(trimmed) && trimmed.length <= 20) return true;
  }
...
export function isEntityFollowUpQuery(message: string): boolean {
  const trimmed = message.trim().toLowerCase();

  if (/^who\s+(is|are|was|were)\s+(he|she|it|they|this|that|this\s+person|this\s+user|this\s+account|this\s+guy|this\s+girl|this\s+dev|this\s+handle|this\s+username|this\s+name)/i.test(trimmed)) return true;
  if (/^what\s+(is|are)\s+(this|that|it|he|she|this\s+thing|this\s+name|this\s+handle)/i.test(trimmed)) return true;
  if (/^tell\s+me\s+(more|about)\s+(him|her|it|this|this\s+person)/i.test(trimmed)) return true;
  if (/^\u0645\u0646\s+(\u0647\u0648|\u0647\u0630\u0647|\u0647\u0630\u0627)\s*(\u0627\u0644\u0634\u062e\u0635|\u0627\u0644\u0645\u0633\u062a\u062e\u062f\u0645|\u0627\u0644\u0627\u0633\u0645|\u0627\u0644\u0627\u0643\u0648\u0646\u062a)?$/i.test(trimmed)) return true;
  if (/^\u0645\u0627\s+\u0647\u0648\s+(\u0647\u0630\u0627|\u0647\u0630\u0647)/i.test(trimmed)) return true;
  if (/^من\s+(هو|هذه|هذا)\s*(الشخص|المستخدم|الاسم|الاكونت)?$/i.test(trimmed)) return true;
  if (/^ما\s+هو\s+(هذا|هذه)/i.test(trimmed)) return true;
  ...
  if (/^(?:ابحث|دور|شوف|فتش)\s+(?:عنه|عنها|عنهم|عنك|عليه|عليها|عليهم|عليك)/i.test(trimmed)) return true;
...
function isFollowUpName(name: string): boolean {
  const lower = name.trim().toLowerCase();
  return PRONOUNS.has(lower);
}

export function isEntityLookupQuery(message: string): boolean {
  ...
  if (whoMatch) {
    ...
    if (isFollowUpName(name)) return false;
    if (name.match(/^(the|a|an|this|that|these|those|my|your|his|her|its|our|their)\s/i)) return false;
    return true;
  }

  if (/^\u0645\u0646\s+\u0647\u0648\s+(.+)/i.test(trimmed)) return true;
  if (/^من\s+هو\s+(.+)/i.test(trimmed)) return true;
  ...
  if (whatMatch) {
    ...
    if (!name) return false;
    if (isFollowUpName(name)) return false;
    if (isCommonProgrammingConcept(name)) return false;
    if (name.match(/^(the|a|an|this|that|these|those|my|your|his|her|its|our|their|a\s+|an\s+)\s/i)) return false;
...
export function extractEntityName(message: string): string | null {
  ...
  if (whatMatch) {
    ...
    if (isCommonProgrammingConcept(name.toLowerCase())) return null;
    return name;
  }

  const arWhoMatch = trimmed.match(/^من\s+هو\s+(.+)/i);
  if (arWhoMatch) return arWhoMatch[1].trim();

  const arWhoMatchEscaped = trimmed.match(/^\u0645\u0646\s+\u0647\u0648\s+(.+)/i);
  if (arWhoMatchEscaped) return arWhoMatchEscaped[1].trim();

  const arWhoMatchUtf8 = trimmed.match(/^من\s+هو\s+(.+)/i);
  if (arWhoMatchUtf8) return arWhoMatchUtf8[1].trim();
...
export function shouldSearchEntity(
  message: string,

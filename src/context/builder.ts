...
let cachedInfo: { data: ProjectInfo; time: number; dir: string } | null = null;
...
export function getProjectInfo(rootDir: string): ProjectInfo {
...
function walkDir(dir: string, rootDir: string, ig: ReturnType<typeof ignore>, allFiles: string[], treeLines: string[]): void {
  let entries: string[];
  try {
...
function analyzeProject(rootDir: string, allFiles: string[], treeLines: string[]): ProjectInfo {
...
function detectProjectType(rootDir: string, files: string[]): string {
  ...
  if (set.has('package.json')) {
    try {
      ...
      if (deps['@nestjs/core'] || deps.nest) return 'NestJS';
  ...
  if (set.has('pyproject.toml') || set.has('setup.py') || set.has('requirements.txt')) {
    if (set.has('manage.py')) return 'Django';
    if (set.has('app.py') || set.has('main.py')) return 'Python (Flask?)';
    return 'Python';
  }
  ...
  if (set.has('composer.json')) return 'PHP';
  if (set.has('pubspec.yaml')) return 'Flutter/Dart';

  return 'Unknown';
}

function detectFramework(type: string): string {
  return type;
}

function findEntryPoints(files: string[], type: string, framework: string): string[] {
...
function findConfigFiles(files: string[]): string[] {
  const fileSet = new Set(files.map(f => f.toLowerCase()));
  return CONFIG_FILE_NAMES.filter(c => fileSet.has(c));
}

function findImportantFiles(files: string[], entryPoints: string[], configFiles: string[]): string[] {
  const important = new Set<string>();
  for (const f of entryPoints) important.add(f);
  for (const f of configFiles) important.add(f);
  for (const f of files) {
    const lower = f.toLowerCase();
    if (lower === 'readme.md') important.add(f);
  }
  return Array.from(important);
}

function buildSummary(type: string, framework: string, entryPoints: string[], configFiles: string[], fileCount: number): string {
  const parts: string[] = [];
  parts.push(`Project type: ${type}`);
  if (entryPoints.length > 0) parts.push(`Entry points: ${entryPoints.join(', ')}`);
  if (configFiles.length > 0) parts.push(`Config files: ${configFiles.join(', ')}`);
  parts.push(`Total files: ${fileCount}`);
  return parts.join(' | ');
}

export function buildProjectTree(rootDir: string): string {

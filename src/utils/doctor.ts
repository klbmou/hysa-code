...
import { loadConfig, PROVIDER_SIGNUP_URLS, PROVIDER_DEFAULTS, PROVIDER_TIERS, TIER_LABELS, FREE_API_PROVIDERS, LOCAL_FREE_PROVIDERS, EXPERIMENTAL_FREE_PROVIDERS, EXPERIMENTAL_BASE_URLS, validateApiKey, normalizeApiKey, providerNeedsApiKey, providerHasOptionalApiKey, getDefaultProviderFromEnv, isLocalFallbackEnabled } from '../config/keys.js';
...
export async function runVisionDiagnostics(debug = false): Promise<void> {
...
export async function runDoctor(debug = false, provider?: string): Promise<void> {
  ...
  if (config) {
    ...
    const hasKey = current === 'ollama' || current === 'local_openai' || current === 'hysa_ai' || providerHasOptionalApiKey(current) || !!config.apiKeys[current as keyof typeof config.apiKeys];
    results.push({

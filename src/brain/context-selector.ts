...
import { detectRecallIntent } from './recall.js';
...
const BUDGETS: Record<TaskComplexity, number> = {
  simple: 800,
  code: 2000,
  planning: 3000,
  provider: 1500,
...
export function detectComplexity(taskKind: string, message: string): TaskComplexity {
...
function scoreRelevance(text: string, keywords: string[]): number {
  if (keywords.length === 0) return 10; // base score for no keywords
...
function scoreRecency(createdAt: string): number {
  ...
  if (age < 7 * DAY_MS) return 80;
  if (age < 30 * DAY_MS) return 50;
  if (age < 90 * DAY_MS) return 20;
  return 5;
}

// ── Format item as string ──

function formatItem(item: ScoredMemoryItem): string {
  ...
  if (node.summary && node.summary !== node.label && !containsSecret(node.summary)) {
    text += `: ${redact(node.summary) as string}`;
  }
...
export async function selectContext(opts: SelectContextOptions): Promise<SelectedContext> {
  ...
  const candidates: ExperienceGraphNode[] = [];
  ...
  for (const n of candidates) {
    ...
    const totalScore = Math.round(
      relevanceScore * 0.4 + importanceScore * 0.25 + confidenceScore * 0.2 + recencyScore * 0.15
    );
  ...
  const selected: ScoredMemoryItem[] = [];
  let totalChars = 0;
  ...
  for (const item of scored) {
    ...
    if (item.node.kind === 'lesson') reasons.push('lesson');
    if (item.node.kind === 'decision') reasons.push('decision');
    if (item.relevanceScore > 20) reasons.push('high-relevance');
    if (item.relevanceScore > 0 && item.relevanceScore <= 20) reasons.push('relevant');

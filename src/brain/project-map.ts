...
const KNOWN_SYSTEM_FILES: Record<string, { system: string; label: string }[]> = {
...
const MODULE_HEURISTICS: Record<string, { purpose: string; system: string }> = {
...
async function scanDir(dir: string, root: string, depth: number = 0): Promise<string[]> {
  if (depth > 4) return [];
...
function detectProjectType(files: string[]): string {
  if (files.some(f => f.startsWith('web/') || f === 'web/index.html')) return 'React + Vite';
  if (files.some(f => f.startsWith('src/') || f === 'index.html')) return 'React';
  if (files.some(f => f.endsWith('.csproj') || f.endsWith('.sln'))) return 'C# / .NET';
  if (files.some(f => f.startsWith('lib/') && (f.endsWith('.dart') || f.endsWith('.rb')))) return 'Dart / Ruby';
  if (existsSync('package.json')) return 'Node.js';
  return 'Unknown';
}

function findImportantFiles(files: string[]): Record<string, string> {
...
function detectModules(files: string[]): Record<string, { purpose: string; files: string[]; dependsOn?: string[] }> {
  ...
  if (modules.provider_config) {
    if (modules.ai_client) modules.provider_config.dependsOn = ['ai_client'];
    if (modules.smart_router) modules.provider_config.dependsOn = ['smart_router'];
  }
...
async function detectCommands(files: string[]): Promise<Record<string, { purpose: string; entryFile?: string }>> {
...
function detectKnownSystems(files: string[]): string[] {
...
export async function generateProjectMap(projectRoot: string): Promise<ProjectMap> {

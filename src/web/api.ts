...
function detectStatsFollowUp(message: string, previousMessage?: string): string | null {
  const trimmed = message.trim();
  if (!trimmed) return null;
  const words = trimmed.split(/\s+/).filter(Boolean);
  if (words.length > 8) return null; // too long to be a follow-up
  if (trimmed.startsWith('/')) return null;
  if (!STATS_FOLLOWUP_RE.test(trimmed)) return null;
  if (!previousMessage) return null;
  const prev = previousMessage.trim();
  if (prev.length > 100 || prev.startsWith('/')) return null;
  const ignoreWords = /^(?:hi|hello|hey|thanks|ok|yes|no|مرحبا|شكرا|نعم|لا)$/i;
  if (ignoreWords.test(prev)) return null;
...
interface ChatResult {
  ...
  searchError?: string;
  searchSources?: { title: string; url: string; snippet?: string; rank: number }[];
  visionDebug?: {
    taskKind: string;
  ...
  try {
    ...
    if (streamSearchQuery) {
      ...
      try {
        ...
        const searchMsg = { role: 'user' as const, content: searchInjection };
        ...
        if (results.length > 0) console.log(`[WebSearch] firstResultTitle="${results[0].title}"`);
        console.log(`[WebSearch] modelPromptIncludesSearchResults=true`);
        const streamSources = results.map((r, i) => ({
          title: r.title,
          url: r.url,
          snippet: (r as any).content || '',
          rank: i + 1,
        }));
      ...
      for (let i = messages.length - 2; i >= 0; i--) {
        if (messages[i].role === 'user' && typeof messages[i].content === 'string') {
          prevMsg = messages[i].content as string;
          break;
        }
      }
...
export async function handleChat(req: ChatRequest): Promise<ChatResult> {
  ...
  try {
    ...
    let searchSources: Array<{ title: string; url: string; snippet: string; rank: number }> | undefined;
    if (searchQuery) {
      const wsDiag = getSearchDiagnostics();
    ...
    return {
      ...
      timing: timingReport,
      searchQuery: (searchQuery || undefined) as string | undefined,
...
function buildUpstreamUrls(encoded: string): string[] {
  return [
...
async function tryFetchUpstream(url: string, signal: AbortSignal, attempt: number): Promise<{
  ok: boolean; buffer?: Buffer; contentType?: string; upstreamUrl: string; status?: number;
  error?: string;
}> {
  try {
...
export async function handleImageProxy(prompt: string): Promise<{
  ok: boolean; buffer?: Buffer; contentType?: string; upstreamUrl?: string; error?: string; status?: number;
}> {
  const encodedPrompt = encodeURIComponent(prompt);

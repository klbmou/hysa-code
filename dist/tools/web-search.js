...
function getConfig() {
    ...
    if (preferredProvider !== 'auto') {
        if (preferredProvider === 'tavily') {
            if (tavilyKey)
                return { provider: 'tavily', tavilyKey };
            return { provider: 'none' };
        }
        if (preferredProvider === 'serper') {
            if (serperKey)
                return { provider: 'serper', serperKey };
            return { provider: 'none' };
        }
        if (preferredProvider === 'brave') {
            if (braveKey)
                return { provider: 'brave', braveKey };
            return { provider: 'none' };
        }
        if (preferredProvider === 'ddg')
            return { provider: 'ddg' };
        return { provider: 'none' };
    }
    if (tavilyKey)
...
export function getSearchDiagnostics() {
    ...
    if (config.tavilyKey)
        configuredKeys.push('Tavily');
    if (config.serperKey)
        configuredKeys.push('Serper');
    if (config.braveKey)
        configuredKeys.push('Brave');
    return {
...
export function isReliableProvider() {
...
export async function searchWeb(query, options) {
    ...
    switch (config.provider) {
        case 'tavily':
            if (!config.tavilyKey)
                throw new Error('Tavily key not configured.');
        ...
        case 'brave':
            if (!config.braveKey)
                throw new Error('Brave key not configured.');

...
export async function upsertNode(node) {
    ...
    if (existingIdx < 0 && (node.kind === 'lesson' || node.kind === 'decision')) {
        ...
        if (similar)
            existingIdx = graph.nodes.findIndex(n => n.id === similar.id);
    }
    if (existingIdx >= 0) {
...
function pickBestSummary(existing, incoming) {
    ...
    if (a === '[REDACTED]' && b !== '[REDACTED]')
        return b;
    if (b === '[REDACTED]' && a !== '[REDACTED]')
        return a;
    return a.length >= b.length ? a : b;
}
function pickHighest(a, b) {
...
// ── Fuzzy dedup helpers ──
function findFuzzyMatch(nodes, kind, label, summary) {
...
export function findDuplicateLabels(nodes) {
    const groups = [];
...
// ── Inspect ──
export async function getInspectReport() {
    const graph = await readExperienceGraph();
    const now = Date.now();
    ...
    const lowImportanceNodes = graph.nodes.filter(n => n.importance !== undefined && n.importance < 30).length;
...
// ── Merge nodes ──
export async function mergeNodes(nodeIds) {
    const graph = await readExperienceGraph();
    const nodes = nodeIds.map(id => graph.nodes.find(n => n.id === id)).filter(Boolean);
    if (nodes.length < 2)
        return null;
    ...
    // Merge summaries
    const summaries = [primary.summary, ...rest.map(n => n.summary)].filter(Boolean);
    ...
    for (const edge of allEdges) {
        if (restIds.has(edge.from))
            edge.from = primary.id;
        if (restIds.has(edge.to))
            edge.to = primary.id;
    }
    // Dedup edges
...
// ── Pin / unpin ──
export async function pinNode(query) {
...
export async function unpinNode(query) {
    const result = await searchGraph(query);
    if (result.nodes.length === 0)
        return null;
    const target = result.nodes.reduce((a, b) => {
...
// ── Forget (remove nodes) ──
export async function forgetNodes(query) {
    const graph = await readExperienceGraph();
    const result = await searchGraph(query);
    const actions = [];
...
export async function cleanupGraph(options = {}) {
    const { dryRun = true, maxAgeDays = 30, minImportance = 30, } = options;
    const graph = await readExperienceGraph();
    const now = Date.now();
    const DAY_MS = 86_400_000;
    const actions = [];
    ...
    for (const n of graph.nodes) {
        ...
        if (n.kind === 'decision' || n.kind === 'lesson')
            continue;
        const age = now - new Date(n.createdAt).getTime();
    ...
    for (const n of graph.nodes) {
        if (n.pinned || toRemove.has(n.id))
            continue;
        if (n.kind !== 'event')
            continue;
        if (!n.label.startsWith('provider_succeeded'))
            continue;
        const age = now - new Date(n.createdAt).getTime();
    ...
    if (!dryRun) {
        const providerEvents = graph.nodes.filter(n => n.kind === 'event' && (n.label.startsWith('provider_failed') || n.label.startsWith('provider_succeeded')));
        const merged = new Set();
        for (let i = 0; i < providerEvents.length; i++) {
            if (merged.has(providerEvents[i].id) || toRemove.has(providerEvents[i].id) || archived.has(providerEvents[i].id))
                continue;
            for (let j = i + 1; j < providerEvents.length; j++) {
                if (merged.has(providerEvents[j].id) || toRemove.has(providerEvents[j].id) || archived.has(providerEvents[j].id))
                    continue;
...
export async function compactGraph(maxNodes = 500) {
    ...
    const priority = (n) => {
        ...
        if (n.pinned)
            return -1;
        // By kind priority
        if (['decision', 'lesson', 'fix', 'bug', 'file', 'provider', 'model'].includes(n.kind))
            return 0;

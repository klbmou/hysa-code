...
function sanitizeSummary(text, maxLen = MAX_SUMMARY_LENGTH) {
...
// ── Exported API ──
export { loadSession };
export function isTrivialSession(state) {
...
export async function recordEvent(kind, detail) {
    ...
    switch (kind) {
        case 'command_run':
            ...
            break;
        case 'file_read':
            if (!state.filesRead.includes(detail))
                state.filesRead.push(detail);
            break;
        case 'file_edited':
            if (!state.filesEdited.includes(detail))
                state.filesEdited.push(detail);
            break;
        case 'tool_used':
            if (!state.toolsUsed.includes(detail))
                state.toolsUsed.push(detail);
            break;
...
export async function generateSummary() {
    ...
    for (const ev of state.events) {
        if (ev.detail.includes('decision') || ev.detail.includes('decided') || ev.detail.includes('Decision')) {
            if (!decisionsMade.includes(ev.detail))
                decisionsMade.push(ev.detail);
        }
        if (ev.detail.includes('lesson') || ev.detail.includes('learned') || ev.detail.includes('Lesson')) {
            if (!lessonsLearned.includes(ev.detail))
                lessonsLearned.push(ev.detail);
        }
        if (ev.kind === 'error_encountered') {
            if (!unresolvedIssues.includes(ev.detail))
                unresolvedIssues.push(ev.detail);
        }
    }
...
export async function formatSummaryForChat() {
...
export async function saveSessionToBrain() {

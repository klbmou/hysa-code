...
export function providerHasCapability(provider, model, capability) {
    const lowerModel = model.toLowerCase();
    for (const entry of CAPABILITY_MAP) {
        if (entry.provider !== provider)
            continue;
        if (entry.model && !lowerModel.includes(entry.model.toLowerCase()))
            continue;
...
export function isModelVisionCapable(model) {
    const lower = model.toLowerCase();
    return CAPABILITY_MAP.some(e => e.model && lower.includes(e.model.toLowerCase()) && e.capabilities.includes('vision'));
}

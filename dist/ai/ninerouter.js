...
export function getNinerouterCandidateRoots(config) {
    ...
    return dedupe(roots.filter((url) => !!url && !!url.trim()).map(normalizeNinerouterRootUrl).flatMap(addIpv4Fallback));
}
...
export function orderNinerouterChatModels(models, preferred) {
    ...
    const ordered = [];
    function add(model) {
        if (!model || !isUsableNinerouterChatModel(model))
            return;
    ...
    for (const model of discovered.filter(isGeminiFlashLiteModel))
        add(model);
    for (const model of discovered.filter(isNvidiaOrFreeModel))
        add(model);
    for (const model of discovered)
        add(model);
    return ordered;
}
export function isUsableNinerouterChatModel(model) {
...
export function extractNinerouterErrorDetails(error) {
    ...
    const rawBody = firstString(record.rawBody, record.body, responseData, parsedError);
...
export function classifyNinerouterFailure(errorOrDetails) {
...
export function ninerouterProbeStatusToErrorCategory(status) {
    switch (status) {
...
export async function probe9RouterModel(config, model, options = {}) {
    ...
    if (config.apiKeys.ninerouter)
        headers.Authorization = `Bearer ${config.apiKeys.ninerouter}`;
    try {
        ...
        const status = classifyNinerouterFailure({
            ...details,
            errorMessage: details.errorMessage || err.message || String(err),
            rawBody: details.rawBody || err.message || String(err),
        });
        return {
            model: normalizedModel,
            usable: false,
            status: status === 'unknown_error' ? 'unavailable' : status,
            reason: details.errorMessage || err.message || String(err),
            latencyMs,
            ...details,
        };
    }
...
function normalizeNinerouterModelId(model) {
    const trimmed = (model || '').trim();
    if (trimmed.startsWith('ninerouter/'))
        return trimmed.slice('ninerouter/'.length);
    return trimmed;
}
...
function isGeminiFlashLiteModel(model) {
    return /^gemini\/.*(?:flash|lite)/i.test(model);
}
function isNvidiaOrFreeModel(model) {
...
function looksLikeErrorDetails(value) {
    if (!isRecord(value))
        return false;

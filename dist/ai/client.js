...
let lastMessagesForTimeout = [];
export function setMessagesForTimeout(messages) {
    lastMessagesForTimeout = messages;
}
function getProviderTimeout(provider, isPrimary, messages) {
    // Env var overrides
...
export function getFallbackCandidates(current, config) {
    ...
    function add(provider, model) {
        ...
        if (!m)
            return;
        if (!providerModelHasActiveCredentials(provider, m, config))
            return;
        const k = `${provider}:${m}`;
        if (added.has(k))
            return;
        ...
        for (const group of groups) {
            ...
            for (const model of group.models) {
                ...
                if (modelResult) {
                    ...
                    if (debug)
                        console.log(`[req:${reqId}] Fallback: ${label} / ${model}`);
                    if (debug) {
